---
title: "Corpus Processing in Arabic"
output:
  html_document:
    fig_caption: yes
...
```{r load_model}
udmodel_arabic <- udpipe_load_model(file = '../models/arabic-padt-ud-2.5-191206.udpipe')
```

## Annotate a sentence

```{r simple_sentence, message=FALSE, warning=FALSE}
txt <- "كيف أصل إلى هناك، من فضلك؟"
annotation <- udpipe_annotate(udmodel_arabic, x = txt)
```
...
news_ar <- read_csv("../data/arabic_news.csv")
news_ar <- news_ar %>%
  mutate(id = paste0(type, '_', id))
```
...
```{r}
corpus_split <- split(sub_corpus, seq(1, nrow(sub_corpus), by = 5))
```
...
annotate_splits <- function(corpus_text) {
  ud_model <- udpipe_load_model("../models/arabic-padt-ud-2.5-191206.udpipe")
...
```{r to_tokens}
sub_tkns <- quanteda::as.tokens(anno_edit, include_pos = "pos", concatenator = "_")
```
...
```{r select_tokens}
sub_dfm <- sub_tkns %>%
  tokens_select("_PUNCT|_X", selection = "remove", valuetype = "regex", case_insensitive = T) %>%
  dfm()
...
```{r subset_corpus}
culture_dfm <- dfm_subset(sub_dfm, text_type == "culture") %>% dfm_trim(min_termfreq = 1)
economy_dfm <- dfm_subset(sub_dfm, text_type == "economy") %>% dfm_trim(min_termfreq = 1)
```

And finally, we can generate a keyness table, 

```{r}
cult_v_econ <- keyness_table(culture_dfm, economy_dfm) %>%
...
```{r echo=FALSE}
knitr::kable(head(cult_v_econ %>% filter(Tag == "noun")))
```
...
```{r}
collocations <- collocates_by_MI(sub_tkns, "الجائزة")
```
